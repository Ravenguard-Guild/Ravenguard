		...
		raids: [
			{
				raid: "The Voidspire",
				bosses: [
					{ name: "Imperator Averzian", cleared: "heroic" },
					{ name: "Vorasius", cleared: "heroic" },
					{ name: "Fallen-King Salhadaar", cleared: "heroic" },
					{ name: "Vaelgor & Ezzorak", cleared: "heroic" },
					{ name: "Lightblinded Vanguard", cleared: "heroic" },
					{ name: "Crown of the Cosmos", cleared: "heroic" },
				],
			},
			{ raid: "The Dreamrift", bosses: [{ name: "Chimaerus, the Undreamt God", cleared: "heroic" }] },
			{
				...
				bosses: [
					{ name: "Belo'ren", cleared: "heroic" },
					{ name: "L'ura, Midnight Falls", cleared: "heroic" },
				],
			},
         ...
         the card, so fill them in whenever the tier details are known). */
			{
				...
				rioKey: "the-venomous-abyss",
				bosses: [
					{ name: "Boss 1", cleared: "none" },
					{ name: "Boss 2", cleared: "none" },
					{ name: "Boss 3", cleared: "none" },
					{ name: "Boss 4", cleared: "none" },
					{ name: "Boss 5", cleared: "none" },
					{ name: "Boss 6", cleared: "none" },
					{ name: "Boss 7", cleared: "none" },
					{ name: "Boss 8", cleared: "none" },
				],
			},
		],
	},
...
var pastTiers = [
	...
	{ id: "sl2", label: "Shadowlands S2", xpac: "Shadowlands — Season 2", raid: "Sanctum of Domination" },
	{
		id: "tww1",
		label: "War Within S1",
		xpac: "The War Within — Season 1",
		raid: "Nerub-ar Palace",
		img: "images/screenshots/aotc-tww.jpg",
	},
];
function summarise(b) {
	var t = b.length,
		nH = b.filter(function (x) {
			return x.cleared === "heroic";
		}).length,
		nA = b.filter(function (x) {
			return x.cleared !== "none";
		}).length;
...
function summariseRio(p) {
	var t = p.total_bosses || 0;
	if (!t) return null;
	var m = p.mythic_bosses_killed || 0,
		hi = Math.max(p.heroic_bosses_killed || 0, m),
		any = Math.max(p.normal_bosses_killed || 0, hi);
...
function summariseFor(rd, rioMap) {
	if (rioMap && rd.rioKey && rioMap[rd.rioKey]) {
		var s = summariseRio(rioMap[rd.rioKey]);
		if (s) return s;
	}
...
var rioData = null,
	activeSeason = null;
/* Default to the most recent season with any manual progress, else the first. */
function defaultSeason() {
	for (var i = seasons.length - 1; i >= 0; i--) {
		if (
			seasons[i].raids.some(function (rd) {
				return rd.bosses.some(function (b) {
					return b.cleared !== "none";
				});
			})
		)
			return seasons[i];
...
function renderRaids() {
	...
	setTimeout(function () {
		f.forEach(function (x) {
			x.el.style.width = x.pct + "%";
		});
	}, 120);
}
function renderSeasonChips() {
	var h = document.getElementById("seasonChips");
	seasons.forEach(function (sn) {
		var c = document.createElement("button");
		c.className = "chip";
		c.dataset.id = sn.id;
		c.textContent = sn.label;
		c.addEventListener("click", function () {
			activeSeason = sn;
			Array.prototype.forEach.call(h.children, function (x) {
				x.classList.toggle("active", x.dataset.id === sn.id);
			});
			renderRaids();
		});
		h.appendChild(c);
	...
	Array.prototype.forEach.call(h.children, function (x) {
		x.classList.toggle("active", x.dataset.id === activeSeason.id);
	});
}
function renderPast() {
	var chips = document.getElementById("tierChips"),
		detail = document.getElementById("tierDetail"),
		panel = document.getElementById("pastPanel");
	function adjust() {
	...
	function show(t) {
		detail.innerHTML =
			'<div class="t-xpac">' +
			t.xpac +
			'</div><div class="t-raid">' +
			t.raid +
			'</div><span class="t-badge">Ahead of the Curve</span>' +
			(t.img ? '<img class="t-shot" src="' + t.img + '" alt="' + t.raid + ' kill" />' : "");
		Array.prototype.forEach.call(chips.children, function (c) {
			c.classList.toggle("active", c.dataset.id === t.id);
		});
		var im = detail.querySelector(".t-shot");
		if (im) {
			im.addEventListener("load", adjust);
		}
		adjust();
	}
	pastTiers.forEach(function (t) {
		var c = document.createElement("button");
		c.className = "chip";
		c.dataset.id = t.id;
...
function fetchRio() {
	var url =
		"https://raider.io/api/v1/guilds/profile?region=" +
		RIO.region +
		"&realm=" +
		RIO.realm +
		"&name=" +
		encodeURIComponent(RIO.name) +
	...
	return fetch(url)
		...
		.then(function (d) {
			return d && d.raid_progression ? d.raid_progression : null;
		})
		.catch(function () {
			return null;
		});
...
(function () {
	...
	renderSeasonChips();
	renderRaids();
	var adjustPast = renderPast();
	fetchRio().then(function (rioMap) {
		if (rioMap) {
			rioData = rioMap;
			renderRaids();
		}
	...
	btn.addEventListener("click", function () {
		...
		panel.style.maxHeight = o ? panel.scrollHeight + "px" : "0";
	});
...
(function () {
	...
	lb.innerHTML =
		'<button class="lb-close" aria-label="Close">&times;</button><div class="lb-stage"><img alt=""></div><div class="lb-hint">Click image to zoom \u00B7 scroll to zoom more \u00B7 Esc to close</div>';
	...
	function setOrigin(e) {
		...
		img.style.transformOrigin =
			clamp(((p.clientX - r.left) / r.width) * 100, 0, 100) +
			"% " +
			clamp(((p.clientY - r.top) / r.height) * 100, 0, 100) +
			"%";
	}
	...
	img.addEventListener(
		...
		function (e) {
			e.preventDefault();
			if (scale <= 1.02) setOrigin(e);
			scale = clamp(scale + (e.deltaY < 0 ? 0.35 : -0.35), 1, 5);
			apply();
		},
		{ passive: false },
	);
	...
	document.addEventListener("click", function (e) {
		var t = e.target;
		if (t.tagName === "IMG" && (t.closest(".killshot") || t.classList.contains("t-shot"))) {
